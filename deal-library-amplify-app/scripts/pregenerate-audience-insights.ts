...
function exportReportsToTypeScript(
  ...
  // Load existing reports if file exists (for merging when using --segment)
  const existingReports = loadExistingReports(outputPath);
  
  // Merge: new reports override existing ones
  const mergedReports = { ...existingReports, ...reports };
 ...
 * Total Reports: ${Object.keys(mergedReports).length}
 * 
...
`;
  ...
  const sortedSegments = Object.keys(mergedReports).sort();
  
  for (const segment of sortedSegments) {
    const report = mergedReports[segment];
  ...
  const newCount = Object.keys(reports).length;
  const totalCount = Object.keys(mergedReports).length;
  if (newCount < totalCount) {
    console.log(`\n✅ Merged ${newCount} new reports with ${totalCount - newCount} existing = ${totalCount} total`);
  } else {
    console.log(`\n✅ Exported ${totalCount} reports to ${outputPath}`);
  }
