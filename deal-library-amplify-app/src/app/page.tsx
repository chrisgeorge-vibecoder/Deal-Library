      ...
      if (cardTypes && cardTypes.length > 1) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            let relevantDeals: Deal[] = [];
            if (cardTypes.includes('deals')) {
              relevantDeals = getRelevantDeals(data.deals || [], query, 6);
            ...
            if (isPersonaDealRequest && relevantDeals.length > 0) {
              // Extract persona name from query for better messaging
              const personaMatch = data.personas?.[0];
              const personaName = personaMatch?.name || personaMatch?.personaName || 'the requested persona';
              setAiResponse(`Found ${relevantDeals.length} deals for ${personaName}. These deals are tailored to reach this specific audience segment. Click on any deal card to see details or add to your cart.`);
            } else if (relevantDeals.length > 0 && cardTypes.includes('deals')) {
              setAiResponse(`Found ${relevantDeals.length} relevant deals matching your query, along with results across ${cardTypes.length} categories.`);
            } else {
