...
export default function AppLayout({ children }: AppLayoutProps) {
  // Default sidebar state: open by default (including desktop)
  ...
  const getCardId = (card: SavedCard | { type: string, data: any }) => {
    switch (card.type) {
      ...
      case 'sovrn-insight':
        return `sovrn-insight-${(card.data as any).id}`;
      default:
        return '';
    }
  };
  ...
  const handleCardClick = (card: SavedCard) => {
    ...
    switch (card.type) {
      ...
      case 'campaign-brief':
        ...
        break;
      case 'sovrn-insight':
        // Navigate to Sovrn Insights page with the insight pre-selected
        router.push(`/sovrn-insights?openInsight=${card.data.id}`);
        break;
      default:
  ...
  const handleClearCart = () => {
    setCart([]);
    // Also clear saved audiences
    const filteredSavedCards = savedCards.filter(card => card.type !== 'audience-taxonomy');
    setSavedCards(filteredSavedCards);
    localStorage.setItem('savedCards', JSON.stringify(filteredSavedCards));
  };

  const handleClearAllSavedCards = () => {
    setSavedCards([]);
  ...
  const showHeader = pathname === '/' || pathname === '/strategy-cards' || pathname === '/audience-insights' || pathname === '/market-pulse' || pathname === '/market-insights' || pathname === '/deals' || pathname === '/audiences' || pathname === '/campaign-planner' || pathname === '/sovrn-insights';
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <Sidebar
        ...
        onOpenSavedCards={() => setIsSavedCardsModalOpen(true)}
      />

      {/* Main Content */}
      <div className={`flex-1 flex flex-col transition-all duration-300 pt-16 md:pt-20 ${sidebarOpen ? 'lg:ml-80' : 'lg:ml-16'}`}>
        {/* Header (conditional) */}
        {showHeader && (
              ...
              <div className="flex justify-between items-center h-16 md:h-20">
                ...
                <div className="flex items-center space-x-1 sm:space-x-2">
                  ...
                  <button
                    ...
                    className="lg:hidden p-2.5 rounded-lg hover:bg-neutral-100 transition-colors touch-manipulation active:scale-95 min-w-[44px] min-h-[44px] flex items-center justify-center"
                  ...
                  <div className="flex items-center space-x-2 sm:space-x-3">
                    <div className="w-12 h-8 sm:w-20 sm:h-10 flex items-center justify-center">
                      <img
                        src="/Sovrn-logo.jpg"
                        alt="Sovrn Logo"
                      ...
                      />
                      <div className="hidden h-8 sm:h-10 w-12 sm:w-20 bg-gradient-to-r from-brand-gold to-brand-orange rounded-lg flex items-center justify-center">
                        <span className="text-brand-charcoal font-bold text-xs sm:text-sm">SOVRN</span>
                      </div>
                    </div>
                    <div>
                      <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-neutral-900">🚀 Launchpad</h1>
                    </div>
                  </div>
                </div>
                
                {/* Right-aligned section: Empty for now - buttons moved to individual tool subheaders */}
                <div className="flex items-center space-x-3">
                  {/* Buttons have been removed from main header - they now appear in individual tool subheaders */}
                </div>
              </div>
            </div>
          </header>
      ...
      {/* Saved Cards Modal */}
      <SavedCardsModal
        isOpen={isSavedCardsModalOpen}
        onClose={() => setIsSavedCardsModalOpen(false)}
        savedCards={savedCards}
        onUnsaveCard={handleUnsaveCard}
        onCardClick={handleCardClick}
        onClearAll={handleClearAllSavedCards}
      />
        ...
        >
          ...
          >
            ...
            <div className="p-4 sm:p-6">
              {cart.length === 0 && savedAudiences.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-neutral-400">Add deals or audiences to your selections to get started</p>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  {cart.length > 0 && (
                    <>
                      <h3 className="text-lg font-semibold text-neutral-900 mb-3">Deals ({cart.length})</h3>
                      {cart.map((deal, index) => (
                        <div key={`${deal.id}-${index}`} className="card flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                          <div className="flex-1">
                            <h3 className="font-semibold text-neutral-900 mb-1 text-sm sm:text-base">{deal.dealName}</h3>
                            <p className="text-xs sm:text-sm text-neutral-600 mb-2">{deal.description}</p>
                  ...
                  {savedAudiences.length > 0 && (
                    <>
                      <h3 className="text-lg font-semibold text-neutral-900 mb-3 mt-6">Audiences ({savedAudiences.length})</h3>
                      {savedAudiences.map((card, index) => (
                        <div key={`${card.data.sovrnSegmentId || card.data.segment?.sovrnSegmentId}-${index}`} className="card flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                          <div className="flex-1">
                            <h3 className="font-semibold text-neutral-900 mb-1 text-sm sm:text-base">
                              {card.data.segmentName || card.data.segment?.segmentName}
                            </h3>
                          ...
                          <button
                            onClick={() => {
                              const cardId = `audience-taxonomy-${card.data.sovrnSegmentId || card.data.segment?.sovrnSegmentId}`;
                  ...
                  <div className="border-t border-neutral-200 pt-4 mt-6">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                      <span className="text-base sm:text-lg font-semibold text-neutral-900">
                        {cart.length + savedAudiences.length} item{cart.length + savedAudiences.length !== 1 ? 's' : ''} selected
                      </span>
