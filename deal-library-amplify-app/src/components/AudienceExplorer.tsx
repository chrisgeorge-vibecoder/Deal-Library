...
export default function AudienceExplorer({ 
  ...
  const renderCard = (result: SearchResult, index: number) => {
    ...
    const category = getCategoryFromType(result.type);
    const style = getStrategyCardStyle(category);
    
    // Get the best title for this card type
    const cardTitle = result.data.dealName || result.data.name || result.data.marketName || 
                      result.data.title || result.data.companyName || result.data.stockSymbol || 
                      result.data.headline || result.data.competitorOrIndustry || 
                      result.data.industryOrTopic || result.data.brandOrCategory || result.data.audienceName;
    
    // Get the best description for this card type
    const cardDescription = result.data.description || result.data.coreInsight || 
                           result.data.summary || result.data.synopsis || 
                           result.data.audienceMotivation || 
                           result.data.messagingFramework?.coreMessage ||
                           result.data.positioning?.currentPerception;
    
    return (
      <div
        key={`${result.type}-${result.data.id}-${index}`}
        onClick={() => handleCardClick(result)}
        className="group relative rounded-xl border border-neutral-200 shadow-sovrn hover:shadow-sovrn-lg transition-all duration-300 overflow-hidden h-full flex flex-col cursor-pointer"
      >
        {/* Colored header with icon and title */}
        <div className={`${style.headerBg} p-4 relative min-h-[88px] flex items-center`}>
          {result.type === 'personas' && result.data.emoji ? (
            <span className="text-3xl bg-white/20 p-2 rounded-xl mr-3">{result.data.emoji}</span>
          ) : (
        ...
        <div className="bg-white p-4 flex flex-col flex-1">
          ...
          {/* Footer indicator */}
          <div className={`flex items-center gap-2 text-xs ${style.text} mt-3 pt-3 border-t border-neutral-100`}>
            ...
            <span>
              ...
              {result.type === 'competitive-intelligence' && 'Competitive analysis'}
              {result.type === 'content-strategy' && 'Content strategy'}
              {result.type === 'brand-strategy' && 'Brand strategy'}
              {result.type === 'deals' && `${result.data.environment} • ${result.data.mediaType}`}
            </span>
          </div>
        </div>
      </div>
