...
import { X, Copy, Check, Sparkles, TrendingUp, Target, Bookmark, BookmarkCheck, Presentation, FileText } from 'lucide-react';
import { CampaignBrief } from '@/types/deal';
import { SlideView, SlideRenderer } from '@/components/slides';
import { generateCampaignBriefSlides } from '@/lib/slideConfigs';
...
export default function CampaignBriefModal({ 
  ...
  // Generate slides for this brief
  const slides = generateCampaignBriefSlides({ ...brief, marketName, marketGeoLevel });
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-0 sm:p-4">
      <div className="bg-white w-full h-full sm:h-auto sm:rounded-xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 bg-gradient-to-r from-brand-gold to-brand-gold-light p-4 sm:p-6 border-b border-brand-gold/20 z-10">
          ...
          {/* View Mode Toggle */}
          <div className="flex items-center gap-1 p-1 bg-white/20 rounded-lg w-fit">
            <button
        ...
        {viewMode === 'slides' ? (
          <div className="min-h-[500px]">
            <SlideView
              slides={slides}
              cardType="campaign-brief"
              cardTitle={`Campaign Brief - ${marketName}`}
            >
