...
export function InlinePersonaCard({ persona, onClick, onSave, isSaved }: InlinePersonaCardProps) {
  return (
    <div 
      onClick={onClick}
      className="group bg-white border border-slate-200/60 rounded-lg p-6 shadow-sm hover:shadow-[0_1px_3px_0_rgba(0,0,0,0.05),0_1px_2px_-1px_rgba(0,0,0,0.05)] hover:-translate-y-0.5 transition-all duration-200 cursor-pointer"
    >
      <div className="flex items-start gap-4">
        <span className="text-3xl flex-shrink-0">{persona.emoji || '👤'}</span>
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-3">
            <h4 className="font-semibold text-slate-900 truncate tracking-tight leading-tight">
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              {onSave && (
                <button
                  ...
                  className={`p-1.5 rounded transition-colors ${isSaved ? 'text-slate-700' : 'text-slate-400 hover:text-slate-600'}`}
                >
                  {isSaved ? <BookmarkCheck className="w-4 h-4" /> : <Bookmark className="w-4 h-4" />}
                </button>
              )}
              <Presentation className="w-4 h-4 text-slate-400 group-hover:text-slate-600 transition-colors" />
              <ChevronRight className="w-4 h-4 text-slate-400 group-hover:text-slate-600 transition-colors" />
            </div>
          </div>
          {persona.category && (
            <span className="inline-block px-2.5 py-1 bg-slate-100 text-slate-700 text-xs font-medium rounded-full mt-2">
              {persona.category}
            </span>
          )}
      ...
      <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
        ...
        <span className="text-slate-600 font-medium group-hover:text-slate-900 transition-colors">View Slides →</span>
      </div>
    </div>
  );
}
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-semibold text-slate-900 tracking-tight leading-tight">{insights.audienceName || 'Audience Insights'}</h4>
            <p className="text-xs text-slate-500 mt-1">Behavioral Analysis</p>
          </div>
        </div>
        <div className="flex items-center gap-1 flex-shrink-0">
          {onSave && (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-semibold text-slate-900 tracking-tight leading-tight">{sizing.marketName || 'Market Intelligence'}</h4>
            <p className="text-xs text-slate-500 mt-1">Market Sizing & Trends</p>
          </div>
        </div>
        <div className="flex items-center gap-1 flex-shrink-0">
          {onSave && (
      ...
      <div className="mt-4 flex items-center gap-6">
        {sizing.totalMarketSize && (
          <div>
            <div className="text-2xl font-semibold text-slate-900 tracking-tight">{sizing.totalMarketSize}</div>
            <div className="text-xs text-slate-500 mt-1">Total Market</div>
          </div>
        )}
        {sizing.growthRate && (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-semibold text-slate-900 tracking-tight leading-tight">{geo.audienceName || 'Geographic Insights'}</h4>
            <p className="text-xs text-slate-500 mt-1">Location Analysis</p>
          </div>
        </div>
        <div className="flex items-center gap-1 flex-shrink-0">
          {onSave && (
      ...
      <div className="mt-4 flex items-center gap-6">
        {geo.totalAddressable && (
          <div>
            <div className="text-xl font-semibold text-slate-900 tracking-tight">{geo.totalAddressable}</div>
            <div className="text-xs text-slate-500 mt-1">Addressable Market</div>
          </div>
        )}
        {topMarket && (
          <div className="text-sm">
            <div className="text-xs text-slate-500 mb-1">Top Market</div>
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-semibold text-slate-900 tracking-tight leading-tight">Marketing SWOT</h4>
            <p className="text-xs text-slate-500 mt-1">{swot.companyName}</p>
          </div>
        </div>
        <div className="flex items-center gap-1 flex-shrink-0">
          {onSave && (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-semibold text-slate-900 tracking-tight leading-tight">Competitive Intelligence</h4>
            <p className="text-xs text-slate-500 mt-1">{intel.competitorOrIndustry}</p>
          </div>
        </div>
        <div className="flex items-center gap-1 flex-shrink-0">
          {onSave && (
      ...
      {intel.competitiveAnalysis?.mainCompetitors && intel.competitiveAnalysis.mainCompetitors.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {intel.competitiveAnalysis.mainCompetitors.slice(0, 3).map((comp, idx) => (
            <span key={idx} className="px-2.5 py-1 bg-slate-100 text-slate-700 text-xs font-medium rounded-full border border-slate-200/60">
              {comp.name}
            </span>
          ))}
          {intel.competitiveAnalysis.mainCompetitors.length > 3 && (
            <span className="px-2.5 py-1 bg-slate-50 text-slate-600 text-xs font-medium rounded-full border border-slate-200/60">
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-semibold text-slate-900 tracking-tight leading-tight">Brand Strategy</h4>
            <p className="text-xs text-slate-500 mt-1">{strategy.brandOrCategory}</p>
          </div>
        </div>
        <div className="flex items-center gap-1 flex-shrink-0">
          {onSave && (
      ...
      {strategy.messagingFramework?.coreMessage && (
        <div className="mt-4 p-4 bg-slate-50 rounded-lg border border-slate-200/60">
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-semibold text-slate-900 tracking-tight leading-tight">Content Strategy</h4>
            <p className="text-xs text-slate-500 mt-1">{strategy.industryOrTopic}</p>
          </div>
        </div>
        <div className="flex items-center gap-1 flex-shrink-0">
          {onSave && (
...
export function InlineCompanyProfileCard({ profile, onClick, onSave, isSaved }: InlineCompanyProfileCardProps) {
  ...
  const recColor = recommendation.includes('buy') ? 'text-slate-700 bg-slate-100 border-slate-200' 
    : recommendation.includes('sell') ? 'text-slate-700 bg-slate-100 border-slate-200' 
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-semibold text-slate-900 tracking-tight leading-tight">
              {profile.companyInfo?.name} ({profile.stockSymbol})
            </h4>
            <p className="text-xs text-slate-500 mt-1">{profile.companyInfo?.sector}</p>
          </div>
        </div>
        <div className="flex items-center gap-1 flex-shrink-0">
          {onSave && (
      ...
      <div className="mt-4 flex items-center gap-4">
        {profile.companyInfo?.marketCap && (
          <div>
            <div className="text-lg font-semibold text-slate-900 tracking-tight">{profile.companyInfo.marketCap}</div>
            <div className="text-xs text-slate-500 mt-1">Market Cap</div>
          </div>
        )}
