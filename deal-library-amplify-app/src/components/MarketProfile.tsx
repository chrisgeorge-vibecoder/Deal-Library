              ...
              <div key={index} className="bg-gradient-to-br from-purple-50 to-pink-50 border border-purple-200 rounded-lg p-4">
                ...
                {onAddToComparison && (
                  <button
                    ...
                    className={`mt-3 w-full flex items-center justify-center gap-2 px-3 py-1.5 border-2 border-blue-500 rounded-lg transition-all duration-200 text-xs font-medium ${
                      comparisonMarketNames.includes(similar.name)
                        ? 'bg-blue-500 text-white'
                        : 'bg-white text-blue-600 hover:bg-blue-500 hover:text-white'
                    }`}
                    title={comparisonMarketNames.includes(similar.name) ? "In comparison" : "Add to comparison"}
                  >
                    <GitCompare className="w-3.5 h-3.5" />
                    {comparisonMarketNames.includes(similar.name) ? 'In Comparison' : 'Compare'}
                  </button>
                )}
              </div>
            ))}
