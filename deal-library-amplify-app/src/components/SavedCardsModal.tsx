...
export default function SavedCardsModal({ 
  ...
  const getCardIcon = (type: string) => {
    switch (type) {
      ...
      case 'sovrn-insight':
        return <Zap className="w-4 h-4 text-brand-gold" />;
      default:
  ...
  const getCardTitle = (card: SavedCard) => {
    switch (card.type) {
      ...
      case 'sovrn-insight':
        return (card.data as any).headline || 'Sovrn Insight';
      default:
        return 'Unknown Card';
    }
  };
  ...
  const getCardSubtitle = (card: SavedCard) => {
    switch (card.type) {
      ...
      case 'sovrn-insight':
        const insight = card.data as any;
        return `${insight.category || 'Insight'} • Sovrn Insights`;
      default:
        return '';
    }
  };
  ...
  return (
    ...
    >
      <div 
        className="bg-white w-full h-full sm:h-auto sm:rounded-xl shadow-sovrn-lg w-full max-w-full sm:max-w-4xl h-full sm:h-auto sm:max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center p-4 sm:p-6 border-b border-neutral-200">
          <h2 className="text-lg sm:text-xl font-bold text-neutral-900">Saved Cards</h2>
